...
class DownloadCog(commands.Cog):
    ...
    @download.sub_command(
        description="Downloads a YouTube video from a link returning the best video under 8mb"
    )
    async def youtube(self, inter: disnake.CmdInter, link: str):
        ...
        try:
            ...
            if len(vids) == 0:
                self.loggerl2.info(
                    f"'{yt.title}' cannot be downloaded due to invaild filters"
                )
                await inter.send(
                    f"'{yt.title}' cannot be downloaded due to invaild filters"
                )
            ...
            if vid.filesize_approx >= 8388608:
                self.loggerl2.info(
                    f"'{yt.title}' cannot be downloaded due to its length and quality\nTry a smaller, or shorter video"
                )
                await inter.send(
                    f"'{yt.title}' cannot be downloaded due to its length and quality"
                )
            ...
            with tempfile.TemporaryDirectory() as td:
                self.loggerl2.info(
                    f"'{inter.user.name}' downloaded '{yt.title}'; size: {round(vid.filesize_approx/1024,5)}kb\ndata: '{vid}'"
                )
    ...
    async def reddit(self, inter: disnake.CmdInter, link: str):
        ...
        downloadA = (
            soup.find("div", {"class": "download-info"}).find("a")
            if not soup.find("div", {"class": "alert alert-danger"})
            else None
        )
                    ...
                    async with self.aiohttp.get(nexthref) as res:
                        await inter.send(
                            file=disnake.File(
                                ...
                                f"{str(time.time()).split('.')[0]}-sound.mp4",
                            )
                        )
                    self.loggerl2.info(
                        f"sound video: {inter.author} downloaded '{link}'"
                    )
                    return
                case "video-no-sound":
                    async with self.aiohttp.get(nexthref) as res:
                        await inter.send(
                            file=disnake.File(
                                io.BytesIO(await res.read()),
                                f"{str(time.time()).split('.')[0]}-no-sound.mp4",
                            )
                        )
                    self.loggerl2.info(
                        f"no sound video: {inter.author} downloaded '{link}'"
                    )
                    return
                case "gif":
...
def setup(bot):
    logging.getLogger("disnakecommands.download").info(
