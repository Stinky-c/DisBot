...
# alias
logconf = tml["logging"]
chance = tml["fun"]["random"]

# disnake logger
logger = logging.getLogger("disnake")
logger.setLevel(logging.INFO)
handler = logging.FileHandler(
    filename=logconf["bot"]["path"],
...
# disnake commands logger
loggercmd = logging.getLogger("disnakecommands")
loggercmd.setLevel(logging.INFO)
handlercmd = logging.FileHandler(
    filename=logconf["commands"]["path"],
    encoding=logconf["encoding"],
    mode=logconf["method"],
)
...
if logconf["verboseenabled"]:
    ...
    async def on_slash_command_error(inter: disnake.CmdInter, error, *args, **kwargs):
        ...
        loggercmd.error(
            f"a command errored in '{inter.guild}' '#{inter.channel}'\nargs: '{args}'kwargs: \n{kwargs}"
        )
...
if tml["fun"]["chat"]:
    ...
    async def on_message(message: disnake.Message):
        if rnd.randint(chance["min"], chance["max"]) == 3 and tml["random"]["enabled"]:
            ...
            loggercmd.info(
                f"'{message.id}' in '{message.channel}' rolled the dice and won\n'{message.jump_url}'"
            )
        # I added it talan
        ...
        # neat
        if "neat" in message:
            await message.reply("...is a mod by vazkii")
